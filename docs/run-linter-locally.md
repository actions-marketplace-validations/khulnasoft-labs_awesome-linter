# Run Awesome-Linter locally to test your branch of code
...
- Pull the latest **Docker** container down from **DockerHub**
  - `docker pull khulnasoft-labs/awesome-linter:latest`
...
- You can run the container locally with the following **Base** flags to run your code:
  - `docker run -e RUN_LOCAL=true -e USE_FIND_ALGORITHM=true -v /path/to/local/codebase:/tmp/lint khulnasoft-labs/awesome-linter`
    - To run against a single file you can use: `docker run -e RUN_LOCAL=true -e USE_FIND_ALGORITHM=true -v /path/to/local/codebase/file:/tmp/lint/file khulnasoft-labs/awesome-linter`
...
### .khulnasoft-labs/awesome-linter.env
...
docker run --rm \
    -e RUN_LOCAL=true \
    --env-file ".khulnasoft-labs/awesome-linter.env" \
    -v "$PWD":/tmp/lint khulnasoft-labs/awesome-linter:v5
```
...
jobs:
  lint:
    ...
    steps:
      ...
      - run: cat .khulnasoft-labs/awesome-linter.env >> "$GITHUB_ENV"
      - name: Lint Code Base
...
- `docker run -it --entrypoint /bin/bash khulnasoft-labs/awesome-linter`
- This will drop you in the command-line of the docker container for any testing or troubleshooting that may be needed.

### Found issues

If you find a _bug_ or _issue_, please open a **GitHub** issue at: [khulnasoft-labs/awesome-linter/issues](https://github.com/khulnasoft-labs/awesome-linter/issues)
