...
LABEL com.github.actions.name="Awesome-Linter Action" \
      com.github.actions.description="Lint your code with GitHub Actions base" \
...
RUN wget --tries=5 https://github.com/cvega/luarocks/archive/v3.3.1-awesome-linter.tar.gz -O - -q | tar -xzf - \
    && cd luarocks-3.3.1-awesome-linter \
    && ./configure --with-lua-include=/usr/local/include \
    && make \
    && make -b install \
    && cd .. && rm -r luarocks-3.3.1-awesome-linter/

RUN luarocks install luacheck

# Install lintr 【 ❛︡ ͜ʖ ❛︠ 】#
COPY --from=lintr-lib /usr/lib/R/library/ /home/r-library
RUN R -e "install.packages(list.dirs('/home/r-library',recursive = FALSE), repos = NULL, type = 'source')"

# Install chktex 【 ❛︡ ͜ʖ ❛︠ 】#
COPY --from=chktex /usr/bin/chktex /usr/bin/
RUN cd ~ && touch .chktexrc

# Load GitHub Env Vars for GitHub Actions 【 ❛︡ ͜ʖ ❛︠ 】#
