...
#      []|[]                                                    #
#      | | |                                                    #
#_____/_]_[_\_________FUNCTION CALLS BELOW______________________#
#### Function LintCodebase 【 ❛︡ ͜ʖ ❛︠ 】#
function LintCodebase() {
  # Pull in the vars 【 ❛︡ ͜ʖ ❛︠ 】#
  ...
  # Validate we have linter installed 【 ❛︡ ͜ʖ ❛︠ 】#
  ...
  # Load the error code 【 ❛︡ ͜ʖ ❛︠ 】#
  ERROR_CODE=$?

  # Check the shell for errors 【 ❛︡ ͜ʖ ❛︠ 】#
  ...
  # Initialize empty Array 【 ❛︡ ͜ʖ ❛︠ 】#
  LIST_FILES=()

  # Set the flag 【 ❛︡ ͜ʖ ❛︠ 】#
  SKIP_FLAG=0

  # Check to see if we need to go through array or all files 【 ❛︡ ͜ʖ ❛︠ 】#
  if [ ${#FILE_ARRAY[@]} -eq 0 ] && [ "${VALIDATE_ALL_CODEBASE}" == "false" ]; then
    ...
    # Set the file separator to newline to allow for grabbing objects with spaces 【 ❛︡ ͜ʖ ❛︠ 】#
    IFS=$'\n'

    # Get list of all files to lint 【 ❛︡ ͜ʖ ❛︠ 】#
    mapfile -t LIST_FILES < <(find "${GITHUB_WORKSPACE}" -path "*/node_modules" -prune -o -type f -regex "${FILE_EXTENSIONS}" 2>&1)

    # Set IFS back to default 【 ❛︡ ͜ʖ ❛︠ 】#
    IFS="${DEFAULT_IFS}"

    # Set it back to empty if loaded with blanks from scanning 【 ❛︡ ͜ʖ ❛︠ 】#
    if [ ${#LIST_FILES[@]} -lt 1 ]; then
      # Set to empty array 【 ❛︡ ͜ʖ ❛︠ 】#
      LIST_FILES=()
      # Skip as we found no files 【 ❛︡ ͜ʖ ❛︠ 】#
      SKIP_FLAG=1
    fi
  fi

  # Check if any data was found 【 ❛︡ ͜ʖ ❛︠ 】#
  if [ ${SKIP_FLAG} -eq 0 ]; then
    # Print Header array 【 ❛︡ ͜ʖ ❛︠ 】#
    for LINE in "${PRINT_ARRAY[@]}"; do
      # Print the header info 【 ❛︡ ͜ʖ ❛︠ 】#
      info "${LINE}"
    done
    ...
    ########################################
    if IsTAP; then
      TMPFILE=$(mktemp -q "/tmp/awesome-linter-${FILE_TYPE}.XXXXXX")
      INDEX=0
      mkdir -p "${REPORT_OUTPUT_FOLDER}"
      REPORT_OUTPUT_FILE="${REPORT_OUTPUT_FOLDER}/awesome-linter-${FILE_TYPE}.${OUTPUT_FORMAT}"
    fi
...
function LintAnsibleFiles() {
  ...
  if [ -d "${ANSIBLE_DIRECTORY}" ]; then
    ...
    # Lint the files 【 ❛︡ ͜ʖ ❛︠ 】#
    for FILE in "${LIST_FILES[@]}"; do

      # Make sure we dont lint certain files 【 ❛︡ ͜ʖ ❛︠ 】#
      if [[ ${FILE} == *"vault.yml"* ]] || [[ ${FILE} == *"galaxy.yml"* ]] || [[ ${FILE} == *"vault.yaml"* ]] || [[ ${FILE} == *"galaxy.yaml"* ]]; then
        # This is a file we dont look at
        continue
      fi

      # Increase the linted file index 【 ❛︡ ͜ʖ ❛︠ 】#
      (("INDEX++"))

      # Get the filename 【 ❛︡ ͜ʖ ❛︠ 】#
      ...
      # Lint the file with the rules 【 ❛︡ ͜ʖ ❛︠ 】#
      ...
      if [ ${ERROR_CODE} -ne 0 ]; then
        ...
        # Store the linting as a temporary file in TAP format 【 ❛︡ ͜ʖ ❛︠ 】#
        if IsTAP; then
          NotOkTap "${INDEX}" "${FILE}" "${TMPFILE}"
          AddDetailedMessageIfEnabled "${LINT_CMD}" "${TMPFILE}"
        fi
    ...
    # Generate report in TAP format 【 ❛︡ ͜ʖ ❛︠ 】#
    if IsTAP && [ ${INDEX} -gt 0 ]; then
      HeaderTap "${INDEX}" "${REPORT_OUTPUT_FILE}"
      cat "${TMPFILE}" >> "${REPORT_OUTPUT_FILE}"
    fi
  else
    # No Ansible dir found 【 ❛︡ ͜ʖ ❛︠ 】#
    warn "No Ansible base directory found at:[${ANSIBLE_DIRECTORY}]"
    debug "skipping ansible lint"
  fi
}
#### Function IsTap 【 ❛︡ ͜ʖ ❛︠ 】#
function IsTAP() {
