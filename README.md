...
[![Codacy Badge](https://app.codacy.com/project/badge/Grade/51071879604e4f319859d4daf91c68f5)](https://app.codacy.com/gh/khulnasoft-labs/awesome-linter/dashboard?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=khulnasoft-labs/awesome-linter&amp;utm_campaign=Badge_Grade)
...
- [Awesome-Linter](#awesome-linter)
  - [Table of Contents](#table-of-contents)
  - [How it Works](#how-it-works)
  - [Supported Linters](#supported-linters)
  - [How to use](#how-to-use)
    - [Example connecting GitHub Action Workflow](#example-connecting-github-action-workflow)
    - [Add Awesome-Linter badge in your repository README](#add-awesome-linter-badge-in-your-repository-readme)
    - [Images](#images)
  ...
  - [Run Awesome-Linter outside GitHub Actions](#run-awesome-linter-outside-github-actions)
...
The awesome-linter finds issues and reports them to the console output. Fixes are suggested in the console output but not automatically fixed, and a status check will show up as failed on the pull request.

The design of the **Awesome-Linter** is currently to allow linting to occur in **GitHub Actions** as a part of continuous integration occurring on pull requests as the commits get pushed. It works best when commits are being pushed early and often to a branch with an open or draft pull request. There is some desire to move this closer to local development for faster feedback on linting errors but this is not yet supported.
...
#      []|[]                                                    #
#      | | |                                                    #
#_____/_]_[_\____________¯ \ _ (ツ) _ / ¯_______________________#
    ...
    steps:
      ...
      - name: Checkout Code
        ...
        with:
          ...
          # list of changed files within `awesome-linter`
          fetch-depth: 0
...
```markdown
[![GitHub Awesome-Linter](https://github.com/<OWNER>/<REPOSITORY>/actions/workflows/<WORKFLOW_FILE_NAME>/badge.svg)](https://github.com/marketplace/actions/awesome-linter)
```
...
The **GitHub Awesome-Linter** now builds and supports `multiple` images. We have found as we added more linters, the image size expanded drastically.
...
The awesome-linter allows you to pass the following `ENV` variables to be able to trigger different functionality.
...
| **IGNORE_GENERATED_FILES**            | `false`                         | If set to `true`, awesome-linter will ignore all the files with `@generated` marker but without `@not-generated` marker.                                                                                               |
| **IGNORE_GITIGNORED_FILES**           | `false`                         | If set to `true`, awesome-linter will ignore all the files that are ignored by Git.                                                                                                                                    |
...
| **LOG_FILE**                          | `awesome-linter.log`              | The filename for outputting logs. All output is sent to the log file regardless of `LOG_LEVEL`.                                                                                                                      |
...
| **SSL_CERT_SECRET**                   | `none`                          | SSL cert to add to the **Awesome-Linter** trust store. This is needed for users on `self-hosted` runners or need to inject the cert for security standards (ex. ${{ secrets.SSL_CERT }})                               |
...
If your repository contains your own rules files that live outside of a `.github/linters/` directory, you will have to tell Awesome-Linter where your rules files are located in your repository, and what their filenames are. To learn more, see [Using your own rules files](docs/using-rules-files.md).
  ...
  build:
    ...
    steps:
      ...
      - name: Checkout Code
        uses: actions/checkout@v3
        with:
          # Full git history is needed to get a proper list of changed files within `awesome-linter`
          fetch-depth: 0
...
Additionally when `IGNORE_GENERATED_FILES=true`, awesome-linter
...
The **Docker** container that is built from this repository is located at [khulnasoft-labs/awesome-linter](https://hub.docker.com/r/khulnasoft-labs/awesome-linter)
...
Check out this [article](https://blog.tyang.org/2020/06/27/use-github-awesome-linter-in-azure-pipelines/)

### GitLab

Check out this [snippet](https://gitlab.com/snippets/1988376) and this Guided Exploration: [GitLab CI CD Extension for Awesome-Linter](https://gitlab.com/guided-explorations/ci-cd-plugin-extensions/ci-cd-plugin-extension-github-action-awesome-linter)
...
Once you have copied the plain text certificate into **GitHub Secrets**, you can use the variable `SSL_CERT_SECRET` to point the **Awesome-Linter** to the files contents.
...
Below are a list of the known limitations for the **GitHub Awesome-Linter**:
...
- Additional details from `package.json` are not read by the **GitHub Awesome-Linter**
